...
use notospace_worker::indexer_worker::{run_background_indexer, BackgroundIndexerConfig};
...
pub async fn create_app(listener: TcpListener, config: Config) -> Result<(), Error> {
  ...
  let tick_interval = get_env_var("notospace_worker_IMPORT_TICK_INTERVAL", "10")
    .parse::<u64>()
    .unwrap_or(10);

  // Maximum file size for import
  let maximum_import_file_size =
    get_env_var("notospace_worker_MAX_IMPORT_FILE_SIZE", "1_000_000_000")
      .parse::<u64>()
  ...
  let indexer_config = BackgroundIndexerConfig {
    enable: NOTO_collaborate::config::get_env_var("NOTO_INDEXER_ENABLED", "true")
      .parse::<bool>()
      .unwrap_or(true),
    open_ai_config,
    azure_ai_config,
