...
const CONSUMER_NAME: &str = "notospace_worker";
  ...
  loop {
      ...
      .await
    {
      ...
      Err(err) => {
        ...
        // Use command:
        // docker exec -it Noto-cloud-redis-1 redis-cli FLUSHDB to generate the error
...
fn is_task_expired(created_timestamp: i64, last_process_at: Option<i64>) -> Result<(), String> {
  match Utc.timestamp_opt(created_timestamp, 0).single() {
    ...
    Some(created_at) => {
      ...
      let hours = get_env_var("notospace_worker_IMPORT_TASK_PROCESS_EXPIRE_HOURS", "6")
        .parse::<i64>()
        .unwrap_or(6);
      ...
      let elapsed = now - created_at;
      let minutes = get_env_var("notospace_worker_IMPORT_TASK_EXPIRE_MINUTES", "30")
        .parse::<i64>()
        .unwrap_or(30);
...
async fn process_task(
  ...
  let retry_interval: u64 = get_env_var("notospace_worker_IMPORT_TASK_RETRY_INTERVAL", "10")
    .parse()
    .unwrap_or(10);

  let streaming = get_env_var("notospace_worker_IMPORT_TASK_STREAMING", "false")
    .parse()
    .unwrap_or(false);
...
async fn download_and_unzip_file(
  ...
  let max_content_length = get_env_var(
    "notospace_worker_IMPORT_TASK_MAX_FILE_SIZE_BYTES",
...
///
/// This function checks whether streaming is enabled via the `notospace_worker_IMPORT_TASK_STREAMING` environment variable.
