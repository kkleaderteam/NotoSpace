use anyhow::Result;
use notospace_worker::error::WorkerError;
use notospace_worker::import_worker::report::{ImportNotifier, ImportProgress};
use notospace_worker::import_worker::worker::{run_import_worker, ImportTask};
use notospace_worker::s3_client::{BlobMeta, S3Client, S3StreamResponse};
...
//   let consumer_name = "notospace_worker";
//   let workspace_id = uuid::Uuid::new_v4().to_string();
...
pub fn setup_log() {
  ...
  START.call_once(|| {
    ...
    let mut filters = vec![];
    filters.push(format!("notospace_worker={}", level));
