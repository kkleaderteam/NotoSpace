...
use NOTO_proto::{Rid, TimestampedEncodedCollab, UpdateFlags};
use bytes::Bytes;
...
impl CollabCache {
  pub fn new(
    ...
    let small_collab_size = get_env_var("NOTO_SMALL_COLLAB_SIZE", "4096")
      .parse::<usize>()
  ...
  pub async fn bulk_insert_collab(
    ...
    let batch_row_size = get_env_var("NOTO_REDIS_BATCH_DB_ROW_COLLAB_SIZE", "50")
      .parse::<usize>()
      .unwrap_or(50);
    ...
    let size = get_env_var("NOTO_REDIS_BATCH_COLLAB_SIZE", "10")
      .parse::<usize>()
      .unwrap_or(10);
  ...
  pub async fn batch_get_snapshot_collab<T: Into<QueryCollab>>(
    ...
    if !db_row_queries.is_empty() {
      let size = get_env_var("NOTO_REDIS_BATCH_DB_ROW_COLLAB_SIZE", "50")
        .parse::<usize>()
        .unwrap_or(50);
