#!/usr/bin/env bash
#
# Noto Cloud Local Development Server
# =====================================
#
# Sets up and runs a complete local development environment for Noto Cloud.
# Includes PostgreSQL database, authentication services, and the Noto Cloud application.
#
# USAGE:
...
#   SKIP_NOTO_CLOUD=true   # Skip Noto Cloud build
...
show_banner() {
    ...
    set +x
    echo -e "${BLUE}=============================================="
    echo -e "  Noto Cloud Local Development Setup"
    echo -e "===============================================${NC}"
    echo ""
    set -x
}
...
set +x
echo -e "${GREEN}=============================================="
echo -e "✓ Noto Cloud Local Development Setup Complete!"
echo -e "==============================================${NC}"
echo ""
echo -e "${CYAN}Services running:${NC}"
echo -e "  ${YELLOW}• PostgreSQL Database:${NC} ${BLUE}localhost:${DB_PORT}${NC}"
echo -e "  ${YELLOW}• Noto Cloud API:${NC} ${BLUE}localhost:9999${NC}"
...
# Build Noto Cloud (unless explicitly skipped)
if [[ -z "${SKIP_NOTO_CLOUD+x}" ]]; then
    set +x
    echo -e "${BLUE}Building Noto Cloud...${NC}"
    set -x
    cargo run --package xtask
    set +x
    echo -e "${GREEN}✓ Noto Cloud build completed${NC}"
    set -x
else
    set +x
    echo -e "${YELLOW}Noto Cloud build skipped (SKIP_NOTO_CLOUD is set)${NC}"
    set -x
fi
