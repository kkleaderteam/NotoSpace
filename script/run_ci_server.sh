...
#===============================================================================
# Noto Cloud CI Server Runner
#===============================================================================
#
# DESCRIPTION:
#   This script builds and runs Noto Cloud services using Docker Compose.
...
# Teardown
docker ps -q --filter "network=notospace-cloud_default" | xargs -r docker stop
docker ps -aq --filter "network=notospace-cloud_default" | xargs -r docker rm
docker compose down
...
if [[ -z "${SKIP_BUILD+x}" ]]; then
  ...
  $BUILD_CLOUD && docker build $BUILD_ARGS \
    -t Notoinc/NOTO_cloud:"$IMAGE_VERSION" \
    -f Dockerfile .
  $BUILD_WORKER && docker build $BUILD_ARGS \
    -t Notoinc/NOTO_worker:"$IMAGE_VERSION" \
    -f services/notospace-worker/Dockerfile .
  $BUILD_ADMIN_FRONTEND && docker build $BUILD_ARGS \
    -t Notoinc/admin_frontend:"$IMAGE_VERSION" \
...
EOF
  ...
  if $BUILD_CLOUD; then
    cat >> docker-compose.override.yml <<EOF
  NOTO_cloud:
    image: Notoinc/NOTO_cloud:$IMAGE_VERSION
EOF
  else
    cat >> docker-compose.override.yml <<EOF
  NOTO_cloud:
    image: Notoinc/NOTO_cloud:latest
EOF
  fi
  ...
  if $BUILD_WORKER; then
    cat >> docker-compose.override.yml <<EOF
  NOTO_worker:
    image: Notoinc/NOTO_worker:$IMAGE_VERSION
EOF
  else
    cat >> docker-compose.override.yml <<EOF
  NOTO_worker:
    image: Notoinc/NOTO_worker:latest
EOF
  fi
  ...
  if $BUILD_ADMIN_FRONTEND; then
    cat >> docker-compose.override.yml <<EOF
  admin_frontend:
    image: Notoinc/admin_frontend:$IMAGE_VERSION
EOF
  else
    cat >> docker-compose.override.yml <<EOF
  admin_frontend:
    image: Notoinc/admin_frontend:latest
EOF
  fi
...
EOF
  ...
  $BUILD_CLOUD && export NOTO_CLOUD_VERSION="$IMAGE_VERSION"
  $BUILD_WORKER && export NOTO_WORKER_VERSION="$IMAGE_VERSION"
  $BUILD_ADMIN_FRONTEND && export NOTO_ADMIN_FRONTEND_VERSION="$IMAGE_VERSION"

  docker compose -f docker-compose-ci.yml pull

  if $BUILD_CLOUD; then
    echo "NOTO_cloud image version:"
    docker images Notoinc/NOTO_cloud --format "{{.Repository}}:{{.Tag}} ({{.CreatedSince}}, {{.Size}})"
  fi

  if $BUILD_WORKER; then
    echo "NOTO_worker image version:"
    docker images Notoinc/NOTO_worker --format "{{.Repository}}:{{.Tag}} ({{.CreatedSince}}, {{.Size}})"
  fi

  if $BUILD_ADMIN_FRONTEND; then
    echo "admin_frontend image version:"
    docker images Notoinc/admin_frontend --format "{{.Repository}}:{{.Tag}} ({{.CreatedSince}}, {{.Size}})"
  fi
