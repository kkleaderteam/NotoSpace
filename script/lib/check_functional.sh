...
# =============================================================================
# Noto Cloud - Diagnostic Tool - Functional Tests
# =============================================================================
# This module handles functional testing of Noto Cloud components including:
...
check_minio_storage() {
    ...
    local bucket_check=$($compose_cmd exec -T minio sh -c 'ls -la /data/Noto' 2>/dev/null)
    if [[ $? -eq 0 ]]; then
        local file_count=$(echo "$bucket_check" | wc -l)
        print_success "Minio: Bucket 'Noto' exists with data"
        print_verbose "Files in bucket: $file_count items"
    else
        print_error "Minio: Bucket 'Noto' not found or inaccessible"
        return 1
    fi
...
check_database_tables() {
    ...
    # Check for essential Noto tables
...
check_websocket_endpoint() {
    ...
    # Extract path from NOTO_WEBSOCKET_BASE_URL (e.g., ws://localhost/ws/v2 -> /ws/v2)
    local ws_path=$(echo "${NOTO_WEBSOCKET_BASE_URL}" | sed 's|^[^/]*//[^/]*/|/|')
    if [[ -z "$ws_path" ]]; then
...
check_websocket_connection_simulation() {
    ...
    if [[ "$response" == "401" || "$response" == "403" ]]; then
        ...
        print_error "  Noto Cloud service may not be running properly"
        print_error "  Check: docker compose ps NOTO_cloud"
        echo ""
        return 1
