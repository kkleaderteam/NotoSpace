...
# =============================================================================
# Noto Cloud - Diagnostic Tool - Report Generation
# =============================================================================
...
generate_report() {
    ...
    {
        echo "=========================================="
        echo "Noto Cloud Diagnostic Report"
        echo "=========================================="
        echo "Generated: $(date)"
        ...
        if [[ -n "$NOTO_CLOUD_VERSION" ]]; then
            echo "Noto Cloud: $NOTO_CLOUD_VERSION"
        else
            echo "Noto Cloud: Unknown"
        fi
...
generate_recommendations() {
    ...
    for issue in "${ISSUES[@]}"; do
        if echo "$issue" | grep -qi "jwt"; then
            ...
            echo "  - Issue: JWT secrets between GoTrue and Noto Cloud don't match"
            echo "  - Fix: Edit .env and ensure GOTRUE_JWT_SECRET equals NOTO_GOTRUE_JWT_SECRET"
        ...
        if echo "$issue" | grep -qi "WebSocket.*wss://.*CRITICAL\|WebSocket URL is ws://"; then
            ...
            echo "  - Fix: Edit .env and change NOTO_WS_BASE_URL to use wss://"
            echo "  - Example: NOTO_WS_BASE_URL=wss://yourdomain.com/ws"
            echo "  - Then restart: docker compose restart NOTO_cloud nginx"
            echo ""
            has_critical=true
        fi
        ...
        if echo "$issue" | grep -qi "Nginx.*Missing.*WebSocket"; then
            ...
            echo "    location /ws {"
            echo "      proxy_pass http://NOTO_cloud:8000;"
        ...
        if echo "$issue" | grep -qi "SMTP Configuration Incomplete"; then
            ...
            echo "  - Issue: GOTRUE_SMTP configured but NOTO_MAILER_SMTP is missing"
            echo "  - Impact: Users CANNOT share workspaces or send invitations"
            echo "  - Common mistake: Configuring only GOTRUE_SMTP (for auth) but forgetting NOTO_MAILER_SMTP (for invitations)"
            echo "  - Fix: Both SMTP configurations are required:"
            echo "    - GOTRUE_SMTP_* for authentication emails (signup, password reset)"
            echo "    - NOTO_MAILER_SMTP_* for workspace invitations and sharing"
            echo "  - Add to .env and restart: docker compose restart NOTO_cloud"
            echo ""
            has_critical=true
        fi
