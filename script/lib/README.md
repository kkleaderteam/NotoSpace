# Noto Cloud Diagnostic Tool - Modular Architecture

This directory contains the modular components of the Noto Cloud diagnostic tool. The original monolithic script has been refactored into separate, maintainable modules.

## Architecture Overview

```
script/
├── diagnose_Noto.sh          # Main orchestration script
└── lib/                           # Module library
...
```bash
# Basic diagnostic
./script/diagnose_Noto.sh

# Verbose with logs
./script/diagnose_Noto.sh -v -l

# Quick check (skip slow tests)
./script/diagnose_Noto.sh --quick

# Custom compose file
./script/diagnose_Noto.sh -f docker-compose-dev.yml
```
...
```bash
# In script/diagnose_Noto.sh main() function
check_my_new_feature
```
...
```
script/diagnose_Noto.sh.backup
```
