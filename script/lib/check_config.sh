...
# =============================================================================
# Noto Cloud - Diagnostic Tool - Configuration Validation
# =============================================================================
    ...
    if [[ -n "$base_url" ]]; then
        ...
        if [[ "$base_scheme" == "https" && "$ws_scheme" != "wss" ]]; then
            ...
            if [[ "$ws_host" != "localhost" && "$ws_host" != "127.0.0.1" ]]; then
                ...
                print_error "  Update .env: NOTO_WS_BASE_URL=wss://yourdomain.com/ws/v2"
...
check_scheme_consistency() {
    ...
    local base_url="${NOTO_BASE_URL}"
    local base_scheme
    base_scheme=$(extract_url_scheme "$base_url")

    if [[ -n "$base_scheme" ]]; then
        if [[ "$base_scheme" != "$scheme_lower" ]]; then
            print_warning "SCHEME=$scheme differs from NOTO_BASE_URL scheme ($base_scheme)"
            print_warning "  Fix: Align SCHEME with the protocol actually served by nginx"
        else
            print_verbose "SCHEME aligns with NOTO_BASE_URL"
        fi
    else
        print_verbose "SCHEME is set but NOTO_BASE_URL scheme could not be determined"
    fi
...
check_duplicate_env_keys() {
    local keys=(
        "NOTO_BASE_URL"
        "API_EXTERNAL_URL"
        "NOTO_GOTRUE_BASE_URL"
        "NOTO_WS_BASE_URL"
        "NOTO_WEBSOCKET_BASE_URL"
        "SCHEME"
    )
...
check_legacy_env_vars() {
    ...
    if [[ -n "$legacy" ]]; then
        print_warning "Detected deprecated AF_* environment variables; update to NOTO_* equivalents"
        if [[ "$VERBOSE" == "true" ]]; then
...
check_gotrue_configuration() {
    ...
    if [[ -z "$site_url" ]]; then
        print_info "GOTRUE_SITE_URL is not set (docker-compose defaults to notospace-flutter:// for desktop/mobile deep links)"
    elif [[ -n "$base_url" ]]; then
        local base_scheme
        base_scheme=$(extract_url_scheme "$base_url")
        local site_scheme
        site_scheme=$(extract_url_scheme "$site_url")
        if [[ "$site_scheme" == "notospace-flutter" ]]; then
            ...
            if [[ -n "$base_scheme" && -n "$site_scheme" && "$base_scheme" != "$site_scheme" ]]; then
                print_warning "GOTRUE_SITE_URL scheme ($site_scheme) differs from NOTO_BASE_URL ($base_scheme)"
            fi
            if [[ $site_url != ${base_url}* ]]; then
                print_warning "GOTRUE_SITE_URL ($site_url) does not start with NOTO_BASE_URL ($base_url)"
            fi
        fi
    fi

    if [[ -n "$uri_allow_list" ]]; then
        if [[ "$uri_allow_list" != *"notospace-flutter://"* ]]; then
            print_warning "GOTRUE_URI_ALLOW_LIST missing notospace-flutter:// callbacks"
        fi
...
check_jwt_secrets() {
    ...
    local NOTO_secret="${NOTO_GOTRUE_JWT_SECRET}"

    # Source code analysis (src/config/config.rs line 213):
    # - NOTO_GOTRUE_JWT_SECRET: Optional, defaults to "hello456" in Noto Cloud code
    # Docker compose analysis (line 117):
    # - NOTO_GOTRUE_JWT_SECRET=${GOTRUE_JWT_SECRET} in container
    # This means:
    # 1. .env only needs GOTRUE_JWT_SECRET
    # 2. Docker compose auto-sets NOTO_GOTRUE_JWT_SECRET from GOTRUE_JWT_SECRET
    # 3. If neither is set, Noto Cloud uses default "hello456"

    # Determine effective Noto secret
    if [[ -z "$NOTO_secret" ]]; then
        # If NOTO_GOTRUE_JWT_SECRET not in .env, docker-compose sets it from GOTRUE_JWT_SECRET
        # If that's also empty, code defaults to "hello456"
        if [[ -n "$gotrue_secret" ]]; then
            NOTO_secret="$gotrue_secret"
            print_verbose "NOTO_GOTRUE_JWT_SECRET not set - will use GOTRUE_JWT_SECRET via docker-compose"
        else
            NOTO_secret="hello456"
            print_info "JWT secrets not configured - using default: $(mask_sensitive "$NOTO_secret")"
    ...
    if [[ -z "$gotrue_secret" ]]; then
        ...
        print_error "    GOTRUE_JWT_SECRET=$NOTO_secret"
        return 1
    fi

    # Check if they match
    if [[ "$gotrue_secret" != "$NOTO_secret" ]]; then
        print_error "JWT secrets do not match (CRITICAL)"
        print_error "  GOTRUE_JWT_SECRET: $(mask_sensitive "$gotrue_secret")"
        print_error "  NOTO_GOTRUE_JWT_SECRET: $(mask_sensitive "$NOTO_secret")"
        print_error "  Fix: Ensure both match in .env:"
        print_error "    GOTRUE_JWT_SECRET=$gotrue_secret"
        print_error "    # NOTO_GOTRUE_JWT_SECRET not needed (auto-set from GOTRUE_JWT_SECRET)"
        return 1
    fi
...
check_database_urls() {
    ...
    local NOTO_db="${NOTO_DATABASE_URL}"
    ...
    if [[ -z "$NOTO_db" ]]; then
        print_error "NOTO_DATABASE_URL is not set"
        return 1
    fi

    # Extract database host from URLs
    local gotrue_host=$(echo "$gotrue_db" | sed -n 's|.*@\([^:/]*\).*|\1|p')
    local NOTO_host=$(echo "$NOTO_db" | sed -n 's|.*@\([^:/]*\).*|\1|p')

    if [[ "$gotrue_host" != "$NOTO_host" ]]; then
        print_warning "Database hosts differ: GoTrue($gotrue_host) vs Noto($NOTO_host)"
    else
        print_success "Database hosts match: $gotrue_host"
    fi

    print_verbose "GOTRUE_DATABASE_URL: $(echo $gotrue_db | sed 's|://.*@|://***:***@|')"
    print_verbose "NOTO_DATABASE_URL: $(echo $NOTO_db | sed 's|://.*@|://***:***@|')"
...
check_base_urls() {
    ...
    local gotrue_base="${NOTO_GOTRUE_BASE_URL}"
    local ws_url_primary="${NOTO_WS_BASE_URL}"
    local ws_url_legacy="${NOTO_WEBSOCKET_BASE_URL}"
    local effective_ws_url="${ws_url_primary:-$ws_url_legacy}"
    local web_url="${NOTO_WEB_URL}"

    # NOTO_WEB_URL is REQUIRED (src/config/config.rs line 274-275)
    # It's the only variable without a default that causes startup failure
    if [[ -z "$web_url" ]]; then
        print_error "NOTO_WEB_URL is not set (REQUIRED)"
        print_error "  Noto Cloud will fail to start without this"
        print_error "  Fix: Add to .env file:"
        print_error "    NOTO_WEB_URL=\${NOTO_BASE_URL}"
        print_error "  For custom web deployment:"
        print_error "    NOTO_WEB_URL=https://your-web-domain.com"
        return 1
    else
        print_success "NOTO_WEB_URL: $web_url"
    fi

    # NOTO_BASE_URL has default in code, but typically set in .env for production
    if [[ -z "$base_url" ]]; then
        print_info "NOTO_BASE_URL not set - check docker-compose for required services"
    else
        print_success "NOTO_BASE_URL: $base_url"
        check_url_scheme_alignment "NOTO_BASE_URL" "$base_url"
    fi

    # API_EXTERNAL_URL typically derived from NOTO_BASE_URL in .env
    ...
    # NOTO_GOTRUE_BASE_URL has default "http://localhost:9999" (src/config/config.rs line 212)
    if [[ -z "$gotrue_base" ]]; then
        print_verbose "NOTO_GOTRUE_BASE_URL not set - using default: http://localhost:9999"
    else
        print_success "NOTO_GOTRUE_BASE_URL: $gotrue_base"
        check_url_scheme_alignment "NOTO_GOTRUE_BASE_URL" "$gotrue_base"
        local gotrue_scheme
        ...
        if [[ -n "$gotrue_host" && "$gotrue_host" != *.* && "$gotrue_scheme" == "https" ]]; then
            print_warning "NOTO_GOTRUE_BASE_URL uses https with internal host '$gotrue_host'"
            print_warning "  Fix: Use http://gotrue:9999 when referencing the container internally"
        fi
    fi

    if [[ -n "$ws_url_primary" && -n "$ws_url_legacy" ]]; then
        print_warning "Both NOTO_WS_BASE_URL and NOTO_WEBSOCKET_BASE_URL are set; NOTO_WS_BASE_URL takes precedence"
    fi

    if [[ -n "$effective_ws_url" ]]; then
        local ws_label
        if [[ -n "$ws_url_primary" ]]; then
            ws_label="NOTO_WS_BASE_URL"
        else
            ws_label="NOTO_WEBSOCKET_BASE_URL"
        fi
        print_success "$ws_label: $effective_ws_url"
        check_websocket_url "$ws_label" "$effective_ws_url" "$base_url"
    else
        print_warning "WebSocket base URL not set (NOTO_WS_BASE_URL or NOTO_WEBSOCKET_BASE_URL)"
    fi
...
check_smtp_configuration() {
    ...
    local NOTO_smtp_host="${NOTO_MAILER_SMTP_HOST}"
    local NOTO_smtp_port="${NOTO_MAILER_SMTP_PORT}"
    local NOTO_smtp_username="${NOTO_MAILER_SMTP_USERNAME}"
    local NOTO_smtp_email="${NOTO_MAILER_SMTP_EMAIL}"

    local has_gotrue_smtp=false
    local has_NOTO_smtp=false
    ...
    # Check if Noto MAILER SMTP is configured
    if [[ -n "$NOTO_smtp_host" && -n "$NOTO_smtp_port" ]]; then
        has_NOTO_smtp=true
        print_success "NOTO_MAILER SMTP configured: $NOTO_smtp_host:$NOTO_smtp_port"
        print_verbose "  Used for: Workspace invitations, sharing notifications"
    fi

    # Critical check: GOTRUE configured but Noto not configured
    if [[ "$has_gotrue_smtp" == "true" && "$has_NOTO_smtp" == "false" ]]; then
        print_error "SMTP Configuration Incomplete (CRITICAL for workspace sharing)"
        print_error "  Issue: GOTRUE_SMTP is configured but NOTO_MAILER_SMTP is not"
        ...
        print_error "  NOTO_MAILER_SMTP is required for workspace invitations"
        print_error ""
        print_error "  Fix: Add these to .env file:"
        print_error "    NOTO_MAILER_SMTP_HOST=$gotrue_smtp_host"
        print_error "    NOTO_MAILER_SMTP_PORT=$gotrue_smtp_port"
        if [[ -n "$gotrue_smtp_user" ]]; then
            print_error "    NOTO_MAILER_SMTP_USERNAME=$gotrue_smtp_user"
            print_error "    NOTO_MAILER_SMTP_EMAIL=$gotrue_smtp_user"
        fi
        print_error "    NOTO_MAILER_SMTP_PASSWORD=<your_smtp_password>"
        print_error "    NOTO_MAILER_SMTP_TLS_KIND=wrapper  # or 'required'"
        print_error ""
        print_error "  Then restart: docker compose restart NOTO_cloud"
        has_issues=true
    fi

    # Warning: Neither configured
    if [[ "$has_gotrue_smtp" == "false" && "$has_NOTO_smtp" == "false" ]]; then
        ...
        print_info "    - NOTO_MAILER_SMTP_* (for workspace invitations)"
    fi

    # Success: Both configured
    if [[ "$has_gotrue_smtp" == "true" && "$has_NOTO_smtp" == "true" ]]; then
        print_success "SMTP: Fully configured for all email features"

        # Check if credentials are set
        if [[ -z "$NOTO_smtp_username" || -z "$NOTO_smtp_email" ]]; then
            print_warning "NOTO_MAILER_SMTP_USERNAME or NOTO_MAILER_SMTP_EMAIL not set"
        fi
    fi

    # Additional validation: Check TLS kind
    local tls_kind="${NOTO_MAILER_SMTP_TLS_KIND}"
    if [[ "$has_NOTO_smtp" == "true" ]]; then
        if [[ -z "$tls_kind" ]]; then
            print_warning "NOTO_MAILER_SMTP_TLS_KIND not set (defaults may not work)"
...
check_ai_server_configuration() {
    ...
    local NOTO_ai_version="${NOTO_AI_VERSION}"
    ...
    if [[ -z "$ai_server_host" && -z "$ai_server_port" ]]; then
        ...
        print_info "    NOTO_AI_VERSION=latest  # or specific version"
        return 0
    fi
    ...
    if [[ -n "$NOTO_ai_version" ]]; then
        print_verbose "  AI Server Version: $NOTO_ai_version"
    else
...
check_nginx_websocket_config() {
    ...
    # Check for WebSocket upgrade headers in location blocks that proxy to Noto
    ...
    # Look for location blocks that handle WebSocket (/ws or upstream NOTO_cloud)
    while IFS= read -r line; do
        ...
        if echo "$line" | grep -E 'upstream\s+(NOTO_cloud|Noto)' &>/dev/null; then
    ...
    if [[ "$has_issues" == "true" ]]; then
        ...
        print_error "    proxy_pass http://NOTO_cloud:8000;"
        print_error "    proxy_http_version 1.1;"
        print_error "    proxy_set_header Upgrade \$http_upgrade;"
        print_error "    proxy_set_header Connection \"upgrade\";"
...
check_production_https_websocket() {
    ...
    local ws_url="${NOTO_WS_BASE_URL:-${NOTO_WEBSOCKET_BASE_URL}}"
    if [[ -z "$ws_url" ]]; then
        print_error "Production HTTPS: WebSocket URL not configured"
        print_error "  Fix: Set NOTO_WS_BASE_URL=wss://$base_host/ws/v2 in .env"
        return 1
    fi
    ...
    if [[ "$ws_scheme" != "wss" ]]; then
        ...
        print_error "    NOTO_WS_BASE_URL=wss://$base_host/ws/v2"
        print_error "  Or if using WS_SCHEME variable:"
        print_error "    WS_SCHEME=wss"
        print_error "  Then restart: docker compose restart NOTO_cloud nginx"
        return 1
    fi
...
check_plan_limits() {
    ...
    # Get NOTO_cloud container ID
    local container_id=$($compose_cmd ps -q NOTO_cloud 2>/dev/null)
    if [[ -z "$container_id" ]]; then
        print_verbose "Noto Cloud container not found"
        return 0
    fi
