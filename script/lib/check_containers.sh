...
# =============================================================================
# Noto Cloud - Diagnostic Tool - Container Checks
# =============================================================================
...
check_container_status() {
    ...
    local services="postgres redis gotrue NOTO_cloud admin_frontend nginx"
...
check_service_versions() {
    ...
    # Check Noto Cloud version
    local NOTO_cloud_container=$($compose_cmd ps -q NOTO_cloud 2>/dev/null)
    if [[ -n "$NOTO_cloud_container" ]]; then
        # Extract version from logs - look for "Using Noto Cloud version:X.X.X"
        NOTO_CLOUD_VERSION=$(docker logs "$NOTO_cloud_container" 2>&1 | grep -oE 'Using Noto Cloud version:[0-9]+\.[0-9]+\.[0-9]+' | head -1 | sed 's/Using Noto Cloud version://')

        if [[ -n "$NOTO_CLOUD_VERSION" ]]; then
            print_success "Noto Cloud version: $NOTO_CLOUD_VERSION"
        else
            print_verbose "Noto Cloud: Version not found in logs"
        fi

        # Check deployment type
        DEPLOYMENT_TYPE=$(docker logs "$NOTO_cloud_container" 2>&1 | grep -oE 'deployment:[a-z-]+' | head -1 | sed 's/deployment://')
        if [[ -n "$DEPLOYMENT_TYPE" ]]; then
            print_verbose "Deployment type: $DEPLOYMENT_TYPE"
        fi
    else
        print_verbose "Noto Cloud: Container not running"
    fi
