...
# =============================================================================
# Noto Cloud - Diagnostic Tool - Log Analysis
# =============================================================================
...
check_admin_frontend_connectivity() {
    print_verbose "Checking Admin Frontend connectivity to Noto Cloud..."
    ...
    # Admin Frontend connects to Noto Cloud via NOTO_BASE_URL
    # Analysis from docker logs and docker-compose.yml:
    # - admin_frontend gets: NOTO_BASE_URL and NOTO_GOTRUE_BASE_URL
    # - It makes API calls to NOTO_BASE_URL (e.g., http://localhost)
    # - nginx proxy routes these to NOTO_cloud backend

    local base_url="${NOTO_BASE_URL}"

    if [[ -z "$base_url" ]]; then
        print_error "Admin Frontend: NOTO_BASE_URL not configured (CRITICAL)"
        print_error "  Admin Frontend cannot connect to Noto Cloud without this"
        return 1
    fi
    ...
    if [[ -n "$connection_errors" ]]; then
        ...
        print_error "  1. NOTO_BASE_URL ($base_url) not accessible from admin_frontend container"
        print_error "  2. Nginx not properly routing requests to NOTO_cloud backend"
        print_error "  3. Noto Cloud service not running or crashed"
        print_error "  4. Network connectivity issues between containers"
        print_error ""
        print_error "Fix steps:"
        print_error "  1. Verify NOTO_BASE_URL in .env matches your deployment"
        print_error "  2. Check nginx is running: docker compose ps nginx"
        print_error "  3. Check NOTO_cloud is running: docker compose ps NOTO_cloud"
...
check_admin_frontend_errors() {
    ...
    if echo "$logs" | grep -qiE "Failed to inject configuration|Configuration error|NOTO_BASE_URL.*undefined"; then
        has_errors=true
        ...
        print_error "  Error: $error_line"
        print_error "  Fix: Verify .env file has correct NOTO_BASE_URL"
        print_error "    Check: NOTO_BASE_URL is set and matches your deployment URL"
        echo ""
    fi
...
check_gotrue_auth_errors() {
    ...
    if [[ -n "$error_logs" ]]; then
        ...
        if [[ "$found_auth_issues" == "true" ]]; then
            ...
            print_error "  2. JWT secret mismatch between GoTrue and Noto Cloud"
...
check_container_errors() {
    ...
    local compose_cmd=$(get_compose_command)
    local services="postgres redis gotrue NOTO_cloud admin_frontend nginx NOTO_worker ai"
    ...
    for service in $services; do
        ...
        if [[ "$should_check_logs" == "true" ]]; then
            ...
            if [[ "$service" == "NOTO_cloud" ]]; then
                local last_version_line=$(echo "$logs" | grep -n "Using Noto Cloud version:" | tail -1 | cut -d: -f1)
                if [[ -n "$last_version_line" ]]; then
                    logs=$(echo "$logs" | tail -n +$last_version_line)
                fi
...
analyze_service_logs() {
    ...
    local compose_cmd=$(get_compose_command)
    local services="gotrue NOTO_cloud nginx admin_frontend"
