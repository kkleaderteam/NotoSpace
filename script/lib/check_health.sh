...
check_health_endpoints() {
    ...
    if [[ -n "$gotrue_port" ]]; then
        ...
        check_health_endpoint "Noto Cloud" "http://localhost:8000/health" 5
    else
        ...
        if curl -s http://localhost/gotrue/health &>/dev/null; then
            ...
            # Also try to check Noto cloud through nginx
            if curl -s http://localhost/api/health &>/dev/null; then
                check_health_endpoint "Noto Cloud (via nginx)" "http://localhost/api/health" 5
            fi
        else
