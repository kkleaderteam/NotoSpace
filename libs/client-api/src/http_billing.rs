...
lazy_static::lazy_static! {
  static ref BASE_BILLING_URL: Option<String> = match std::env::var("notospace_cloud_BASE_BILLING_URL") {
    Ok(url) => Some(url),
    Err(err) => {
      tracing::warn!("std::env::var(notospace_cloud_BASE_BILLING_URL): {}", err);
      None
    },
  };
}
