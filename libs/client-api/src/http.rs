...
const DESKTOP_CALLBACK_URL: &str = "Noto-flutter://login-callback";
...
impl Client {
  ...
  /// If not, the user will be redirected to the Noto-flutter:// by default
  ///
  /// The redirect_to should be the scheme of the app, e.g., Noto-flutter://
  #[instrument(level = "debug", skip_all, err)]
  ...
  /// It looks like, e.g., `Noto-flutter://#access_token=...&expires_in=3600&provider_token=...&refresh_token=...&token_type=bearer`.
  ///
  ...
  /// a deep link to the Noto app (iOS, macOS, etc.), which will call [Client::sign_in_with_url] to sign in.
  ///
  /// For example, the OAuth URL on Google looks like `https://Noto.io/authorize?provider=google`.
  /// The deep link looks like `Noto-flutter://#access_token=...&expires_in=3600&provider_token=...&refresh_token=...&token_type=bearer`.
  ///
  ///
  /// # Parameters
  ...
  /// which calls gotrue authentication server, which then redirects to the Noto-flutter:// with the authentication token.
  ///
