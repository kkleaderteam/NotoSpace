...
async fn main() -> Result<()> {
  ...
  let notospace_cloud_bin_name = "notospace_cloud";
  let worker_bin_name = "notospace_worker";
  ...
  if no_worker {
    println!("Worker disabled - running without notospace-worker");
  }

  // Step 1: Kill existing processes
  let kill_cloud_result = kill_existing_process(notospace_cloud_bin_name);
  ...
  if no_worker {
    println!("Starting Noto-cloud only...");
  } else {
  ...
  let notospace_cloud_handle = tokio::spawn(async move {
    let cmd = spawn_server("cargo", &cloud_args, notospace_cloud_bin_name)?;
    wait_for_readiness(notospace_cloud_bin_name).await?;
    Ok::<_, anyhow::Error>(cmd)
  });

  let notospace_worker_handle = if no_worker {
    None
  } else {
    let mut worker_args = vec!["run"];
    worker_args.extend(&profile_flags);
    worker_args.extend(&["--manifest-path", "./services/notospace-worker/Cargo.toml"]);
  ...
  let mut notospace_cloud_cmd = notospace_cloud_handle.await??;
  let notospace_worker_cmd = if let Some(handle) = notospace_worker_handle {
    Some(handle.await??)
  } else {
    None
  };
  ...
  if no_worker {
    println!("Noto Cloud is up and running (worker disabled).");
  } else {
  ...
  match notospace_worker_cmd {
    Some(mut worker_cmd) => {
      // Monitor both cloud and worker
      select! {
          status = notospace_cloud_cmd.wait() => {
              handle_process_exit(status?, notospace_cloud_bin_name)?;
          },
