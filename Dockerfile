...
RUN echo "Building notospace cloud with profile: ${PROFILE}"
...
RUN if [ "$PROFILE" = "release" ]; then \
      cargo build --release --features "${FEATURES}" --bin notospace_cloud; \
    else \
      cargo build --features "${FEATURES}" --bin notospace_cloud; \
    fi
...
COPY --from=builder /app/target/$PROFILE/notospace_cloud /usr/local/bin/notospace_cloud
ENV APP_ENVIRONMENT production
ENV RUST_BACKTRACE 1

ARG NOTO_APPLICATION_PORT
ARG PORT
ENV PORT=${NOTO_APPLICATION_PORT:-${PORT:-8000}}
EXPOSE $PORT
