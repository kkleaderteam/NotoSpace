...
use crate::api::metrics::NotoWebMetrics;
...
use NOTO_collaborate::ws2::{CollabUpdatePublisher, WorkspaceCollabInstanceCache};
use chrono::DateTime;
...
pub async fn update_space(
  ...
  update_workspace_folder_data(
    &state.metrics.NOTO_web_metrics,
    &state.ws_server,
    user,
    workspace_id,
    folder_update,
...
pub async fn publish_page(
  ...
  let metadata = PublishViewMetaData {
    ...
    // Noto web as there is now endpoint to obtain published outline.
    child_views: vec![],
    ancestor_views: vec![],
  };
...
async fn update_collab_data_with_timeout(
  NOTO_web_metrics: &NotoWebMetrics,
  ...
  NOTO_web_metrics.record_update_size_bytes(update.len());
  ...
  .await
  .map_err(|err| {
    NOTO_web_metrics.incr_apply_update_timeout_count(1);
    AppError::Internal(anyhow!(
  ...
  match resp {
    Ok(_) => Ok(()),
    Err(err) => {
      NOTO_web_metrics.incr_apply_update_failure_count(1);
...
async fn get_all_user_uuids_with_guest_access_to_page(
  ...
  // Note: the open source version of Noto Cloud does not support guest access, hence this will always return empty.
  Ok(vec![])
}
