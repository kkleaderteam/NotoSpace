...
use NOTO_ai_client::client::NotoAIClient;
use NOTO_collaborate::actix_ws::server::RealtimeServerActor;
use NOTO_collaborate::collab::cache::CollabCache;
use NOTO_collaborate::collab::collab_store::CollabStoreImpl;
use NOTO_collaborate::ws2::{CollabManager, WsServer};
use NOTO_collaborate::CollaborationServer;
...
pub async fn init_state(config: &Config) -> Result<AppState, Error> {
  ...
  info!("Setup Noto AI: {}", config.NOTO_ai.url());
  let NOTO_ai_client = NotoAIClient::new(&config.NOTO_ai.url());
  // Pg listeners
  ...
  let use_redis_ac_cache = get_env_var("NOTO_ACCESS_CONTROL_REDIS_CACHE_ENABLED", "false")
    .parse::<bool>()
    .unwrap_or(false);
  ...
  if config.notification.enable_email_notification {
    ...
    let task_NOTO_web_url = config.NOTO_web_url.clone();
  ...
  let embedder_config = IndexerConfiguration {
    enable: get_env_var("NOTO_INDEXER_ENABLED", "true")
      .parse::<bool>()
      .unwrap_or(true),
    open_ai_config,
    azure_ai_config,
    embedding_buffer_size: NOTO_collaborate::config::get_env_var(
      "NOTO_INDEXER_EMBEDDING_BUFFER_SIZE",
      "5000",
    )
    .parse::<usize>()
    .unwrap_or(5000),
