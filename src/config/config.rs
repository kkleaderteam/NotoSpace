...
pub fn get_configuration() -> Result<Config, anyhow::Error> {
  ...
  let config = Config {
    app_env: get_env_var("NOTO_ENVIRONMENT", "local")
      .parse()
      .context("fail to get NOTO_ENVIRONMENT")?,
    access_control: AccessControlSetting {
      is_enabled: get_env_var("NOTO_ACCESS_CONTROL", "false")
        .parse()
        .context("fail to get NOTO_ACCESS_CONTROL")?,
      enable_workspace_access_control: get_env_var("NOTO_ACCESS_CONTROL_WORKSPACE", "true")
        .parse()
        .context("fail to get NOTO_ACCESS_CONTROL_WORKSPACE")?,
      enable_collab_access_control: get_env_var("NOTO_ACCESS_CONTROL_COLLAB", "true")
        .parse()
        .context("fail to get NOTO_ACCESS_CONTROL_COLLAB")?,
      enable_realtime_access_control: get_env_var("NOTO_ACCESS_CONTROL_REALTIME", "true")
        .parse()
        .context("fail to get NOTO_ACCESS_CONTROL_REALTIME")?,
    },
    db_settings: DatabaseSetting {
      ...
      require_ssl: get_env_var("NOTO_DATABASE_REQUIRE_SSL", "false")
        .parse()
        .context("fail to get NOTO_DATABASE_REQUIRE_SSL")?,
      max_connections: get_env_var("NOTO_DATABASE_MAX_CONNECTIONS", "40")
        .parse()
        .context("fail to get NOTO_DATABASE_MAX_CONNECTIONS")?,
    },
    gotrue: GoTrueSetting {
      base_url: get_env_var("NOTO_GOTRUE_BASE_URL", "http://localhost:9999"),
      jwt_secret: get_env_var("NOTO_GOTRUE_JWT_SECRET", "hello456").into(),
      service_role: get_env_var("NOTO_GOTRUE_SERVICE_ROLE", "service_role"),
    },
    application: ApplicationSetting {
      port: get_env_var("NOTO_APPLICATION_PORT", "8000").parse()?,
      host: get_env_var("NOTO_APPLICATION_HOST", "[::]"),
    },
    websocket: WebsocketSetting {
      heartbeat_interval: get_env_var("NOTO_WEBSOCKET_HEARTBEAT_INTERVAL", "6").parse()?,
      client_timeout: get_env_var("NOTO_WEBSOCKET_CLIENT_TIMEOUT", "60").parse()?,
      min_client_version: get_env_var("NOTO_WEBSOCKET_CLIENT_MIN_VERSION", "0.5.0").parse()?,
    },
    redis_uri: get_env_var("NOTO_REDIS_URI", "redis://localhost:6379").into(),
    redis_worker_count: get_env_var("NOTO_REDIS_WORKERS", "60").parse()?,
    s3: S3Setting {
      create_bucket: get_env_var("NOTO_S3_CREATE_BUCKET", "true")
        .parse()
        .context("fail to get NOTO_S3_CREATE_BUCKET")?,
      use_minio: get_env_var("NOTO_S3_USE_MINIO", "true")
        .parse()
        .context("fail to get NOTO_S3_USE_MINIO")?,
      minio_url: get_env_var("NOTO_S3_MINIO_URL", "http://localhost:9000"),
      access_key: get_env_var("NOTO_S3_ACCESS_KEY", "minioadmin"),
      secret_key: get_env_var("NOTO_S3_SECRET_KEY", "minioadmin").into(),
      bucket: get_env_var("NOTO_S3_BUCKET", "Noto"),
      region: get_env_var("NOTO_S3_REGION", ""),
      presigned_url_endpoint: get_env_var_opt("NOTO_S3_PRESIGNED_URL_ENDPOINT"),
    },
    NOTO_ai: NotoAISetting {
    ...
    collab: CollabSetting {
      group_persistence_interval_secs: get_env_var(
        "NOTO_COLLAB_GROUP_PERSISTENCE_INTERVAL",
        "60",
      )
      .parse()?,
      group_prune_grace_period_secs: get_env_var("NOTO_COLLAB_GROUP_GRACE_PERIOD_SECS", "60")
        .parse()?,
      edit_state_max_count: get_env_var("NOTO_COLLAB_EDIT_STATE_MAX_COUNT", "100").parse()?,
      edit_state_max_secs: get_env_var("NOTO_COLLAB_EDIT_STATE_MAX_SECS", "60").parse()?,
      s3_collab_threshold: get_env_var("NOTO_COLLAB_S3_THRESHOLD", "8000").parse()?,
    },
    published_collab: PublishedCollabSetting {
      storage_backend: get_env_var("NOTO_PUBLISHED_COLLAB_STORAGE_BACKEND", "postgres")
        .as_str()
        .try_into()?,
    },
    mailer: MailerSetting {
      smtp_host: get_env_var("NOTO_MAILER_SMTP_HOST", "smtp.gmail.com"),
      smtp_port: get_env_var("NOTO_MAILER_SMTP_PORT", "465").parse()?,
      smtp_username: get_env_var("NOTO_MAILER_SMTP_USERNAME", "sender@example.com"),
      smtp_email: get_env_var("NOTO_MAILER_SMTP_EMAIL", "sender@example.com"),
      smtp_password: get_env_var("NOTO_MAILER_SMTP_PASSWORD", "password").into(),
      smtp_tls_kind: get_env_var("NOTO_MAILER_SMTP_TLS_KIND", "wrapper"),
    },
    apple_oauth: AppleOAuthSetting {
      client_id: get_env_var("NOTO_APPLE_OAUTH_CLIENT_ID", ""),
      client_secret: get_env_var("NOTO_APPLE_OAUTH_CLIENT_SECRET", "").into(),
    },
    NOTO_web_url: get_env_var_opt("NOTO_WEB_URL")
      .ok_or(anyhow!("NOTO_WEB_URL has not been set"))?,
    notification: NotificationSetting {
      enable_email_notification: get_env_var("NOTO_NOTIFICATION_ENABLE_EMAIL", "false")
        .parse()?,
      email_notification_interval_secs: get_env_var(
        "NOTO_NOTIFICATION_EMAIL_INTERVAL_SECS",
        "900",
      )
      .parse()?,
      email_notification_grace_period_secs: get_env_var(
        "NOTO_NOTIFICATION_EMAIL_GRACE_PERIOD_SECS",
        "450",
      )
      .parse()?,
    },
