...
fn page_router() -> Router<AppState> {
  Router::new()
    ...
    .route(
      "/open-Noto-or-download",
      get(open_NOTO_or_download_handler),
    )
...
async fn open_NOTO_or_download_handler() -> Result<Html<String>, WebAppError> {
  render_template(templates::OpenNotoOrDownload {})
}
...
async fn login_callback_query_handler(
        ...
        None => match query.error {
          Some(err) => {
            ...
            let redirect_url = format!(
                "https://Noto.io/invitation/expired?workspace_name={}&workspace_icon={}&user_name={}&user_icon={}&workspace_member_count={}",
  ...
  verify_token_cloud(
    ...
    state.notospace_cloud_url.as_str(),
  )
  .await?;
  ...
  match query.action {
    Some(action) => match action {
      OAuthLoginAction::AcceptWorkspaceInvite => {
        ...
        {
          // If user has already accepted the invitation, redirect to open or download Noto
          let accepted_invitations = get_accepted_workspace_invitations(
            &new_session.token.access_token,
            &state.notospace_cloud_url,
          )
          .await?;
          ...
          if found.is_some() {
            let open_or_dl_html = render_template(templates::OpenNotoOrDownload {})?;
...
async fn shared_workspaces_handler(
  ...
  let user_workspaces =
    get_user_workspaces(&session.token.access_token, &state.notospace_cloud_url).await?;
...
async fn user_usage_handler(
  ...
  let workspace_count =
    get_user_owned_workspaces(&session.token.access_token, &state.notospace_cloud_url)
      .await
  ...
  let workspace_limit =
    get_user_workspace_limit(&session.token.access_token, &state.notospace_cloud_url)
      .await
...
async fn workspace_usage_handler(
  ...
  let workspace_usages =
    get_user_workspace_usages(&session.token.access_token, &app_state.notospace_cloud_url).await?;
