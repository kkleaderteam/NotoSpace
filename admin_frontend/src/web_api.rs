...
/// Generates a URL to facilitate login redirection to the Noto app from a web browser.
///
/// This function creates a custom URL scheme that can be used in a web browser to open the
/// Noto app and automatically handle user login based on the provided `UserSession`.
///
/// # Returns
/// A `Result` containing `HeaderMap` for HTTP redirection if successful, or `WebApiError` in case of failure.
///
/// # Example URL Format
/// `Noto-flutter://login-callback#access_token=...&expires_at=...&expires_in=...&refresh_token=...&token_type=...`
///
...
async fn delete_account_handler(
  ...
  delete_current_user(&session.token.access_token, &state.notospace_cloud_url).await?;
