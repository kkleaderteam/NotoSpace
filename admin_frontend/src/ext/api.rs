...
pub async fn get_user_owned_workspaces(
  access_token: &str,
  notospace_cloud_base_url: &str,
) -> Result<Vec<AFWorkspace>, Error> {
  let user_profile = get_user_profile(access_token, notospace_cloud_base_url).await?;
  let owned_workspaces = get_user_workspaces(access_token, notospace_cloud_base_url)
    .await?
    .into_iter()
...
pub async fn get_user_workspaces(
  ...
  let resp = http_client
    .get(format!("{}/api/workspace", notospace_cloud_base_url))
...
pub async fn get_user_workspace_limit(
  ...
  let http_client = reqwest::Client::new();
  let resp = http_client
    .get(format!("{}/api/user/limit", notospace_cloud_base_url))
    .header("Authorization", format!("Bearer {}", access_token))
    .send()
    .await?;
...
pub async fn get_user_workspace_usages(
  ...
  let user_workspaces = get_user_owned_workspaces(access_token, notospace_cloud_base_url).await?;
  ...
  for user_workspace in user_workspaces {
    ...
    let members =
      get_workspace_members(&workspace_id, access_token, notospace_cloud_base_url).await?;
    let total_blob_size =
      get_user_workspace_blob_usage(&workspace_id, access_token, notospace_cloud_base_url)
        .await
    ...
    let total_doc_size = {
      get_user_workspace_doc_usage(&workspace_id, access_token, notospace_cloud_base_url)
        .await
...
pub async fn get_workspace_members(
  ...
  let resp = http_client
    .get(format!(
      ...
      notospace_cloud_base_url, workspace_id
    ))
    .header("Authorization", format!("Bearer {}", access_token))
    .send()
    .await?;
...
async fn get_user_workspace_blob_usage(
  workspace_id: &str,
  access_token: &str,
  notospace_cloud_gateway_base_url: &str,
) -> Result<WorkspaceBlobUsage, Error> {
  let http_client = reqwest::Client::new();
  let resp = http_client
    .get(format!(
      "{}/api/file_storage/{}/usage",
      notospace_cloud_gateway_base_url, workspace_id
    ))
    .header("Authorization", format!("Bearer {}", access_token))
    .send()
    .await?;
...
pub async fn get_user_profile(
  ...
  let http_client = reqwest::Client::new();
  let url = format!("{}/api/user/profile", notospace_cloud_base_url);
  let resp = http_client
    .get(url)
    .header("Authorization", format!("Bearer {}", access_token))
    .send()
...
pub async fn accept_workspace_invitation(
  ...
  let url = format!(
    ...
    notospace_cloud_base_url, invite_id
  );
  let resp = http_client
    .post(url)
    .header("Authorization", format!("Bearer {}", access_token))
...
pub async fn verify_token_cloud(
  ...
  let url = format!(
    "{}/api/user/verify/{}",
    notospace_cloud_base_url, access_token
  );
  let resp = http_client
    .get(url)
    .header("Authorization", format!("Bearer {}", access_token))
...
pub async fn delete_current_user(
  ...
  let http_client = reqwest::Client::new();
  let url = format!("{}/api/user", notospace_cloud_base_url);
  let resp = http_client
    .delete(url)
    .header("Authorization", format!("Bearer {}", access_token))
    .send()
