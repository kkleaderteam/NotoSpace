...
pub struct Config {
  ...
  pub notospace_cloud_url: String,
...
impl Config {
  pub fn from_env() -> Result<Config, anyhow::Error> {
    let cfg = Config {
      ...
      notospace_cloud_url: get_or_default(
        "ADMIN_FRONTEND_notospace_cloud_URL",
        "http://localhost:8000",
      ),
      oauth: OAuthConfig {
        client_id: get_or_default("ADMIN_FRONTEND_OAUTH_CLIENT_ID", "notospace_cloud"),
